  ...
  try {
    ...
    if (chatId > 0)
      await bot.telegram.sendMessage(
        LOG_CHANNEL_ID,
        `Пользователю ${await getUserLinkById(chatId)} отправлено\n\n` +
          message,
        { parse_mode: 'HTML' }
      )
    return true
    ...
    if (
      ...
      await bot.telegram.sendMessage(
        LOG_CHANNEL_ID,
        `Чат не найден: <code>${chatId}</code>`,
        { parse_mode: 'HTML' }
      )
    }
    return false
...
function formatMessage(comment, total) {
  const { id_task, kolvo_brak, det_name, type, comments_otk, specs_nom_id } =
    comment
...
async function processUserComments(userComments, userName) {
  ...
  if (isMessageSent) {
    ...
    if (masterChatId) {
      const masterMessage = formatMasterMessage(
        userComments[0],
        chatId,
        userName
      )
...
async function notifyAllUsers() {
  ...
  for (let chatId of user_ids) {
    ...
    const userComments = allComments.filter(
      (comment) => comment.user_id === chatId && comment.sent === 0
    )
...
async function notifyUsers(ctx) {
  ...
  const userUnansweredComments = uncommentedTasks.filter(
    ({ user_id, sent }) => user_id === chatId && sent === 0
  )
