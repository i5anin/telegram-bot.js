...
async function handleAddComment(ctx) {
  ...
  if (matchTaskID && matchDetName) {
    ...
    const comment = comments.find(
      (c) => c.id_task === Number(taskID) && c.user_id === chatId
    )
    ...
    if (response && response.status === 'OK') {
      ...
      if (comment.user_id_master)
        await bot.telegram.sendMessage(comment.user_id_master, master_msg, {
          parse_mode: 'HTML'
        })
    } else {
      console.error(
        'Ошибка на стороне сервера:',
        response.message || response.errorMessage || 'Неизвестная ошибка'
      )
      throw new Error('Ошибка на стороне сервера')
    }
  } else {
    console.log(
      'Invalid task_ID format or missing detName in the reply message!'
    )
  }
}
