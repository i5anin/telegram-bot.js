...
async function payments(ctx) {
    try {
        ...
        await ctx.telegram.sendMessage(LOG_CHANNEL_ID, `Пользователю ${await getUserLinkById(userId)} отправлено\n\n` + message, {
            parse_mode: 'HTML',
        })
    } catch (error) {
        const logMessageToSend = {
...
async function handleFormulaButton(ctx) {
    try {
        ...
        if (paymentData) {
            ...
            await ctx.telegram.sendMessage(
                LOG_CHANNEL_ID, `Пользователю ${await getUserLinkById(userId)} отправлено\n\n` +
